...
// 动态生成的映射表（从 code.json 自动构建）
let CATEGORY_MAP = {};
let FUND_CODES_MAP = {};
let CODE_NAME_MAP = {};

function buildMapsFromCodeConfig() {
    CATEGORY_MAP = {};
    FUND_CODES_MAP = {};
    CODE_NAME_MAP = {};

    for (const category of Object.keys(codeConfig)) {
        const indices = codeConfig[category];
        if (!Array.isArray(indices)) continue;

        for (const item of indices) {
            if (!item.code) continue;

            CATEGORY_MAP[item.code] = category;
            CODE_NAME_MAP[item.code] = item.name || item.code;
            FUND_CODES_MAP[item.code] = {
                场内代码: item.场内代码 || '-',
                场外代码: item.场外代码 || '-'
            };
        }
    }
    console.log('[基金温度] 已从 code.json 构建映射表');
}
...
function calculateAndShowStockWeather() {
    ...
    if (codeConfig['B']) {
        for (const item of codeConfig['B']) {
            const code = item.code;
            const data = fundsData[code];
            if (data) {
    ...
    if (codeConfig['C']) {
        for (const item of codeConfig['C']) {
            const code = item.code;
            const data = fundsData[code];
            if (data) {
