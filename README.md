...
- 精选数据行用金色边框和浅蓝色底色标记
- **股市晴雨表**：显示大盘/小盘PE温度和PB温度，提供入市建议
- **中证全指EPV**：计算PE_TTM除以10年期国债收益率，提供投资建议（黄金坑、白银坑等）
- **四大魔盒**：包含中证800PE、中证800PB、股市吸引力、巴菲特指数、七日换手率
- **集中配置**：所有指数配置集中在 `code.json` 文件中，添加新指数只需修改此文件
- **自动同步**：上传CSV文件到GitHub后，网站会自动加载最新数据
...
├── js/
│   └── app.js         # 主脚本文件
├── code.json          # 指数配置文件（集中管理所有指数）
├── bond_yield.txt     # 10年期国债收益率数据
├── README.md          # 项目说明文档
...
### Q: 如何添加新的指数？
A: 只需编辑 `code.json` 文件，添加新的指数对象：
```json
{"code": "指数代码", "name": "指数名称", "场内代码": "场内代码", "场外代码": "场外代码"}
```
无需修改其他文件，网站会自动加载新指数。

### Q: 如何更新10年期国债收益率？
A: 编辑 `bond_yield.txt` 文件，输入最新的收益率（如 `2.55%`），上传到GitHub后网站会自动更新。
...
### Q: 如何更新指数类别？
A: 编辑 `code.json` 文件，指数按类别分组（A-债券、B-大盘、C-小盘、D-策略、E-行业、F-主题、G-海外）。

### Q: 为什么股市晴雨表显示Infinity%？
A: 这是因为计算时出现了除以零的情况。请确保CSV文件中有正确的PE和PB分位点数据。

### Q: 为什么10年期国债收益显示错误页面？
A: 这是因为 `bond_yield.txt` 文件不存在或HTTP请求失败。代码已添加错误处理，会使用默认值 `1.803%`。
...
### 2026-05-30
- **集中配置优化**：重构代码，所有指数配置集中在 `code.json` 文件中
- 添加新指数只需修改 `code.json`，无需修改其他文件
- **股市晴雨表修复**：修复了PE温度和PB温度显示Infinity%的问题
- **10年期国债收益修复**：修复了文件不存在时显示404错误页面的问题
- 添加了HTTP错误处理，文件不存在时使用默认值
- **项目清理**：删除了不必要的文件（爬虫脚本、数据文件、文档等）
- 只保留核心文件：`index.html`、`css/style.css`、`js/app.js`、`code.json`、`bond_yield.txt`
- **GitHub部署优化**：支持上传CSV文件后自动同步更新
